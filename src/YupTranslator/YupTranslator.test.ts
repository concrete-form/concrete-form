...
import YupTranslator from './YupTranslator'
...
describe('YupTranslator', () => {
  it('replaces mixed validations', async () => {
    ...
    const expected = [
      ...
      { key: TranslationKeys.ONE_OF, values: { values: ['foo', 'bar'] }, meta: { from: 'yup', type: 'mixed', validation: 'oneOf' } },
      { key: TranslationKeys.NOT_ONE_OF, values: { values: ['a', 'b', 'c'] }, meta: { from: 'yup', type: 'mixed', validation: 'notOneOf' } },
    ]

    expect(received).toEqual(expected)
  })

  it('replaces string validations', async () => {
    const schema = new YupTranslator(Yup.object().shape({
  ...
  it('uses "default" when field type doesn\'t match', async () => {
    ...
    const received = await getErrors(schema, invalid)
    const expected = [
      { key: TranslationKeys.DEFAULT, meta: { from: 'yup', type: 'string', validation: 'typeError' } },
    ]
  ...
  it('use "default" for custom validations', async () => {
    ...
    const received = await getErrors(schema, invalid)
    const expected = [
      { key: TranslationKeys.DEFAULT, meta: { from: 'yup', type: 'string', validation: 'custom' } },
    ]

    expect(received).toEqual(expected)
  })

  it('use "default" for other unsupported validations', async () => {
    const schema = new YupTranslator(Yup.object().shape({
    ...
    const received = await getErrors(schema, invalid)
    const expected = [
      { key: TranslationKeys.DEFAULT, meta: { from: 'yup', type: 'string', validation: 'uuid' } },
    ]
  ...
  it('handle nested objects type missmatch', async () => {
    ...
    const received = await getErrors(schema, invalid)
    const expected = [
      { key: TranslationKeys.DEFAULT, meta: { from: 'yup', type: 'object', validation: 'typeError' } },
    ]
    expect(received).toEqual(expected)
  })
